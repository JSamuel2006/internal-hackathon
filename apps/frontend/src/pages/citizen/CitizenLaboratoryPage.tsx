  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto px-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-900 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Track sample collections, view completed reports, and read AI critical result alerts</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 glass-panel p-6 rounded-2xl border border-slate-900 space-y-4 self-start">
          ...
          <div className="space-y-3">
            {orders.map((ord) => (
              ...
              >
                ...
                {ord.result && (
                  <p className="text-[10px] text-slate-400 bg-slate-950 p-2 rounded border border-slate-900 leading-normal">{ord.result}</p>
                )}
              </div>
            ))}
          </div>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              {selectedOrder.status !== 'Completed' && (
                <div className="glass-panel p-6 rounded-2xl border border-slate-900 space-y-4">
                  <h4 className="text-xs font-bold text-slate-205 uppercase tracking-wider border-b border-slate-850 pb-2">
                  ...
                  <div className="space-y-3">
                    <input
                      ...
                      className="bg-slate-950 border border-slate-850 rounded-xl p-2.5 text-xs text-slate-355 w-full focus:outline-none font-mono"
                    />
                    <button
              ...
              {aiReport && (
                <div className="glass-panel p-6 rounded-2xl border border-slate-900 space-y-4 text-xs font-mono animate-fade-in">
                  ...
                  <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-900 text-slate-300 leading-relaxed">
                    {aiReport.summary}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-900">
                      ...
                      <strong className="text-slate-300 block mt-1">{aiReport.repeatInvestigationRecommended ? 'YES' : 'NO'}</strong>
                    </div>
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-900">
                      <span className="text-slate-500 text-[9px] uppercase block">Next Steps</span>
                      <strong className="text-slate-300 block mt-1">{aiReport.nextSteps}</strong>
                    </div>
                  </div>
                </div>
              )}
