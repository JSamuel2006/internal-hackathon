    ...
    <div className="space-y-6 max-w-6xl mx-auto px-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Your complete chronological healthcare activity history</p>
        </div>
        <button 
          onClick={fetchTimeline} 
          className="self-start px-3.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-lg text-xs font-mono flex items-center gap-1.5 border border-slate-800 transition-colors"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-panel p-4 rounded-xl border border-slate-900 bg-slate-950/40">
          <span className="text-[10px] uppercase font-mono tracking-wider text-slate-500 block">Total Activity</span>
          <p className="text-2xl font-bold font-mono text-slate-100 mt-1">{loading ? '...' : totalItems}</p>
        </div>
        <div className="glass-panel p-4 rounded-xl border border-slate-900 bg-slate-950/40">
      ...
      <div className="glass-panel p-4 rounded-xl border border-slate-900 bg-slate-950/20 flex flex-col gap-4">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500/50 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 outline-none transition-all placeholder:text-slate-500"
            />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <select 
              ...
              className="bg-slate-900 border border-slate-800 rounded-xl px-2 py-2 text-xs text-slate-350 outline-none focus:border-rose-500/50"
            >
      ...
      {loading ? (
        // Skeleton Loader
        <div className="space-y-6 ml-4 pl-6 border-l border-slate-900 py-4">
          {[1, 2, 3].map((n) => (
            <div key={n} className="space-y-4 animate-pulse relative">
              ...
              <div className="glass-panel p-5 rounded-2xl border border-slate-900 bg-slate-950/20 space-y-3">
        ...
        // Error State
        <div className="glass-panel p-8 rounded-2xl border border-slate-900 text-center space-y-4 bg-rose-950/5">
          <AlertCircle className="w-12 h-12 text-rose-500 mx-auto" />
          <h3 className="text-base font-bold text-slate-200">Unable to load your health timeline</h3>
          <p className="text-xs text-slate-450 max-w-sm mx-auto">Your existing health records are safe. Please check your connection or try again.</p>
          <button 
            onClick={fetchTimeline} 
        ...
        // Empty State
        <div className="glass-panel p-12 rounded-2xl border border-slate-900 text-center space-y-4 bg-slate-950/30">
          ...
          <button 
            ...
            className="px-6 py-2 bg-slate-900 hover:bg-slate-850 text-slate-200 border border-slate-850 rounded-xl text-xs font-semibold transition-colors"
          >
          ...
          {Object.keys(groupedEvents).map((dateLabel) => (
            <div key={dateLabel} className="space-y-4">
              ...
              <div className="relative -left-[32px] flex items-center gap-2">
                <div className="w-3.5 h-3.5 rounded-full bg-slate-950 border-2 border-rose-500 glow-pill shrink-0"></div>
                ...
                {groupedEvents[dateLabel].map((e) => {
                    ...
                    metadataExtra = (
                      <div className="space-y-2 text-[11px] font-mono text-slate-500 bg-slate-950/50 p-3 rounded-lg border border-slate-900">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                          <div>Medicine: <span className="text-slate-200 font-semibold">{parsedJson?.medicineName || 'Not detected'}</span></div>
                          <div>Generic: <span className="text-slate-200 truncate block">{parsedJson?.genericName || 'Not detected'} • {parsedJson?.strength || 'Not detected'}</span></div>
                  ...
                  return (
                    <div key={e.id} className="relative group">
                      {/* Connection node */}
                      <div className="absolute -left-[31px] top-4 w-2.5 h-2.5 rounded-full bg-slate-900 border border-slate-700 group-hover:border-rose-500 group-hover:bg-rose-500/10 transition-colors"></div>
                      
                      {/* Event Card */}
                      <div className="glass-panel p-5 rounded-2xl border border-slate-900 hover:border-slate-800 transition-all space-y-3 bg-slate-950/20">
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-slate-900 rounded-xl border border-slate-850 flex items-center justify-center shrink-0">
                              {getEventIcon(e.type)}
                            </div>
                            <div>
                              <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider block">{e.displayType}</span>
                              <h4 className="text-sm font-bold text-slate-200 group-hover:text-slate-100 transition-colors">{e.title}</h4>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-900/60">
                          ...
                          <button 
                            ...
                            className="px-3 py-1 bg-slate-900 hover:bg-slate-850 hover:text-rose-400 text-slate-300 border border-slate-850 rounded-lg text-[10px] font-semibold transition-colors"
                          >
                            View Details
                          </button>
                        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-900 pt-5">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              disabled={page === 1}
              className="p-1.5 bg-slate-900 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg border border-slate-850 text-slate-300"
            >
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg h-full bg-[#070b19] border-l border-slate-850 p-6 flex flex-col justify-between overflow-y-auto space-y-6">
            <div className="space-y-5">
              <div className="flex items-center justify-between pb-3 border-b border-slate-850">
                ...
                <button 
                  ...
                  className="p-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-450 hover:text-slate-200"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {selectedEvent.type === 'MEDICINE_SCAN' && (() => {
                ...
                const getVal = (val: string) => {
                  if (!val || val === 'Unable to Detect' || val === 'Unknown' || val === '') {
                    return { text: 'Not detected', badge: <span className="px-2 py-0.5 rounded text-[9px] font-mono bg-slate-900 text-slate-500 border border-slate-800">? Not Detected</span> };
                  }
                ...
                return (
                  <div className="space-y-5 text-xs text-slate-350">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">OVERVIEW</h4>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center col-span-2">
                          <div>
                            <span className="text-[9px] text-slate-500 block font-mono">MEDICINE NAME</span>
                            <span className="font-semibold text-slate-200">{medName.text}</span>
                          </div>
                          {medName.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center col-span-2">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200">{genName.text}</span>
                          </div>
                          {genName.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            <span className="text-[9px] text-slate-500 block font-mono">STRENGTH</span>
                            <span className="font-semibold text-slate-200">{strength.text}</span>
                          </div>
                          {strength.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200">{dosage.text}</span>
                          </div>
                          {dosage.badge}
                        </div>
                      </div>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">PACKAGING VALIDATION</h4>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center col-span-2">
                          <div>
                            <span className="text-[9px] text-slate-500 block font-mono">MANUFACTURER</span>
                            <span className="font-semibold text-slate-200">{manufacturer.text}</span>
                          </div>
                          {manufacturer.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200 font-mono">{batch.text}</span>
                          </div>
                          {batch.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200">{mrp.text}</span>
                          </div>
                          {mrp.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200">{mfgDate.text}</span>
                          </div>
                          {mfgDate.badge}
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 flex justify-between items-center">
                          <div>
                            ...
                            <span className="font-semibold text-slate-200">{expDate.text}</span>
                          </div>
                          {expDate.badge}
                        </div>
                      </div>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">OCR ANALYSIS</h4>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900">
                          ...
                          <span className="font-bold text-slate-200 font-mono">{(selectedEvent.metadata.confidenceScore || 91)}%</span>
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900">
                          <span className="text-[9px] text-slate-500 block font-mono">FIELD COVERAGE</span>
                          <span className="font-bold text-slate-200 font-mono">90%</span>
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 col-span-2 flex justify-between items-center">
                          <div>
                            <span className="text-[9px] text-slate-500 block font-mono">WINNING OCR PASS</span>
                            <span className="text-slate-200 font-mono">Pass #1 (Tesseract OCR Engine)</span>
                          </div>
                        </div>
                        <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 col-span-2 flex justify-between items-center">
                          <div>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">VERIFICATION</h4>
                      <div className="grid grid-cols-3 gap-2">
                        <div className="p-2 bg-slate-950/60 rounded-xl border border-slate-900 text-center space-y-1">
                    ...
                    {selectedEvent.metadata.geminiAnalysis && (
                      <div className="space-y-3">
                        <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">SAFETY & CLINICAL ANALYSIS</h4>
                        <div className="border border-slate-900 rounded-xl p-3 bg-slate-950/20 space-y-2">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-mono tracking-widest text-rose-455 font-bold uppercase border-b border-slate-900 pb-1">TRACEABILITY</h4>
                      <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1 font-mono text-[9px] text-slate-500">
                ...
                <div className="space-y-4 text-xs">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                      ...
                      <p className="font-semibold text-slate-200">Multisystem Risk Engine</p>
                    </div>
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                  ...
                  <div className="border border-slate-900 rounded-xl p-3.5 bg-slate-950/20 space-y-2">
                    <h4 className="font-bold text-slate-200 text-xs border-b border-slate-900 pb-1.5">Model Outputs & Recommendations</h4>
                    <p className="text-slate-400 text-xs leading-relaxed">{selectedEvent.metadata.specialistRecommended ? `Recommended Specialist: ${selectedEvent.metadata.specialistRecommended}` : 'No abnormal risks detected.'}</p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4 text-xs">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                      ...
                      <p className="font-semibold text-slate-200">Dr. {selectedEvent.metadata.doctorName}</p>
                    </div>
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                      <span className="text-[9px] text-slate-500 font-mono block">FACILITY</span>
                      <p className="font-semibold text-slate-200">{selectedEvent.metadata.hospitalName}</p>
                    </div>
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                      <span className="text-[9px] text-slate-500 font-mono block">APPOINTMENT DATE</span>
                      <p className="font-semibold text-slate-200">{selectedEvent.metadata.date}</p>
                    </div>
                    <div className="p-3 bg-slate-950/60 rounded-xl border border-slate-900 space-y-1">
                      <span className="text-[9px] text-slate-500 font-mono block">SCHEDULED TIME</span>
                      <p className="font-semibold text-slate-200">{selectedEvent.metadata.time}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <button 
              onClick={() => setSelectedEvent(null)}
              className="w-full py-2.5 bg-slate-900 hover:bg-slate-850 text-slate-200 border border-slate-800 rounded-xl text-xs font-semibold transition-colors"
            >
              Close Details View
            </button>
          </div>
