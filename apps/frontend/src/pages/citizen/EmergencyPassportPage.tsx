    ...
    <div className="space-y-8 max-w-4xl mx-auto px-4 font-mono text-xs text-slate-350">
      <div className="border-b border-slate-200 pb-4 flex justify-between items-center">
        ...
        <button
          ...
          className="p-2 border border-slate-200 bg-white rounded-xl flex items-center gap-2"
        >
      ...
      {passport ? (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-7 bg-white border border-slate-200 shadow-sm p-6 rounded-2xl border border-slate-200 space-y-4">
            <h3 className="text-slate-202 font-bold uppercase tracking-wider border-b border-slate-200 pb-2">EHR Clinical Context Summary</h3>
            <div className="space-y-3">
              <div>
                <span className="text-[10px] text-slate-500 uppercase block">Chronic Diseases</span>
                <p className="text-slate-700 font-semibold">{passport.chronicDiseases.join(', ') || 'None reported'}</p>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase block">Current Medications</span>
                <p className="text-slate-700 font-semibold">{passport.currentMedicines.join(', ') || 'None active'}</p>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase block font-bold">Digital Twin parameters</span>
                <p className="text-slate-700 font-semibold">Cardiac safety: {passport.digitalTwinSummary.cardiacScore}%</p>
              </div>
            </div>
          </div>
        </div>
