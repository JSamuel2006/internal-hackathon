  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 border-b border-slate-900 pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-100">National Health Report Center</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-900 md:col-span-1 flex flex-col justify-between h-80">
          <form onSubmit={handleGenerate} className="space-y-4">
            <h3 className="text-xs font-mono font-bold tracking-wider text-slate-500 uppercase">Configuration</h3>

            <div>
              <label className="block text-[10px] font-mono text-slate-450 uppercase mb-1.5">Target District</label>
              <input
                type="text"
                required
                value={district}
                onChange={(e) => setDistrict(e.target.value)}
                className="w-full px-3 py-2 bg-slate-900 border border-slate-800 text-xs rounded-lg text-slate-200 outline-none focus:border-teal-500/35"
              />
            </div>

            <div>
              <label className="block text-[10px] font-mono text-slate-450 uppercase mb-1.5">Report Period</label>
              <input
                type="month"
                required
                value={month}
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-900 md:col-span-2 space-y-4 min-h-[400px] flex flex-col justify-between">
          <div>
              ...
              {report && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 rounded hover:bg-slate-900 text-slate-400 hover:text-white transition-colors"
            ...
            {report ? (
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-slate-900/50 border border-slate-850 text-xs text-slate-350 leading-relaxed font-mono whitespace-pre-wrap max-h-[350px] overflow-y-auto">
                  {report.reportContent}
                </div>
              </div>
            ) : (
              <div className="h-64 border border-dashed border-slate-900 rounded-xl flex flex-col items-center justify-center text-slate-500 text-center p-6">
